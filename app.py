...
app = dash.Dash(
    __name__,server=server,   external_stylesheets=[dbc.themes.SUPERHERO],
    meta_tags=[
        {
            'charset': 'utf-8',
        },
...
#https://drive.google.com/drive/folders/1d74w492LsyldeNdcEcrhlaz9fE8cYPtE?usp=sharing
...
header_text=html.Div('Energy Transition Dashboard',style=dict(color='white',
...
#dbc.Spinner([map_div],size="lg", color="primary", type="border", fullscreen=False )
...
app.layout=html.Div([ dbc.Row([db_logo_img,db_header_text],style=dict(backgroundColor='#20374c') )
                      ,dbc.Row([db_navigation_header])  , html.Br()  ,dbc.Row( id='layout'),
                        dcc.Store(id='area_chart_updated', data='done'),
                        dcc.Store(id='pie_chart_updated', data='done')
...
# for more advanced info on callbacks : https://dash.plotly.com/advanced-callbacks

@app.callback([Output('gen_area_chart','figure'),Output('area_chart_updated','data')],
...
@app.callback(Output('gen_bar_chart','figure'),
               [Input('bar_country_menu','value'),Input('bar_resolution_menu','value'),Input('planets_list','value'),Input('pie_chart_updated','data')]

              )
def update_gen_bar_chart(selected_countries,selected_resolution,selected_assets,pie_chart_updated):
...
@app.callback([Output('pie_chart','figure'),Output('pie_chart_updated','data')],
               [Input('pie_country_menu','value'),Input('pie_slider','value'),Input('area_chart_updated','data')]
#,prevent_initial_call=True
              )
def update_pie_chart(selected_countries,years_range,area_chart_updated):
